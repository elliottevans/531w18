...
\item
  ...
  \todo[inline]{For $X_1,X_2,\dots$ iid with density $f_X(x)$, then to calculate $\int h(x)f_X(x)\,dx=E(h(X))\approx \frac{1}{n}\sum_{i=1}^{n}h(X_i)$ where $X\sim f_X$.}
  
  \todo[inline]{$Z_1,Z_2,\dots\sim f_Z(z)$, we have $\int h(x)f_X(x)\,dx = \int h(x)\frac{f_X(x)}{f_Z(x)}f_Z(x)\,dx\approx \frac{1}{n}\sum_{i=1}^{n}h(Z_i)\frac{f_X(Z_i)}{f_Z(Z_i)}$ where $\frac{f_X(Z_i)}{f_Z(Z_i)}$ are the importance weights. Here, $f_Z$ is $G$ from above.}
  
  \todo[inline]{For us, $h(x) = f_{Y|X}(y^*|x)$. $\int h(x)f_X(x)\, dx = \int f_{Y|X}(y^*|x)f_X(x)\,dx=\int f_{XY}(x,y^*)\, dx = f_Y(y^*)$.}
...
\todo[inline]{Above, demeasure with log=TRUE provides the log-likelihood. We see this since $exp(ell) \approx 5.37e-102\implies ell = \ln(5.37e-102)\approx -233$ }
\item
...
\item
  ...
  \todo[inline]{Here, $J$ is the monte carlo sample size.}
\end{itemize}
...
\todo[inline]{Standard error of 0.8277 is now much smaller than before.}
...
\item
  ...
  \todo[inline]{This is the perturbtion density}
\item
...
\todo[inline]{The dt above is $\Delta t$ (see notes 11.10) }
...
\todo[inline]{The delta.t choice is somewhat arbitrary. It 1/12 just means that the euler method will take 12 steps between time points. We don't want it to be any smaller than it needs to be. The smaller we make this, the longer the computation time.}
...
<<<<<<< HEAD
\todo[inline]{Below, \%d.rda, runlevel is inserting the runlevel parameter into the character string. This is the implementation of the IF2 algorithm in 12.7.1. Here, Np is $J$ in the pseudo code, }
=======
\todo[inline]{Below, \%d.rda, runlevel is inserting the runlevel parameter into the character string}
...
<<<<<<< HEAD
\todo[inline]{Below, in the top plot, there's consistency b/t the difference global searches.}
