...
\todo[inline]{They are both monotone over the same intervals.}
...
\item
  ...
  \todo[inline]{No expected value because this is the observed information, plugging in MLE instead of taking expectation.}
...
\item
  ...
  \todo[inline]{Profile likelihood gives the same parameter estimate, but the CI it generates is better when the likelihood is multimodal or isn't perfectly smooth. Here, $\hat{\theta}^*$ is the MLE. This contains all the $\theta_d$ such that the log-likelihood evaluated at the MLE is within 1.92 of the profile log-likelihood at $\theta_d$.}
\item
...
\todo[inline]{This is because Fisher information is based on quadratic approximation. Thus, profile likelihood CIs are good for flat likelihood.}
\end{itemize}
...
\todo[inline]{Use likelihood-ratio test for smaller v. larger model. Use AIC to pick the better predictive model.}
...
\todo[inline]{Lowest AIC: ARMA(2,1) and ARMA(1,0). Can't necessarily say one of these is better (they both seem reasonable). We can suggest that models with no AR (autoregressive term) are bad (have higher AICs).}
...
\todo[inline]{AIC is not a hypothesis test - it's not that formal. Only tries to compare predictive skill of models.}
...
\todo[inline]{The vertical distance in the plot is much less than 1.92, so profile likelihood tells us that we don't know where on the curve the estimate should be. This suggests there is an identifiability issue and the SE should be larger than the Fisher information estimate. Next thing to do is a simulation study - this seems to match with the profile likelihood.}
...
\todo[inline]{Look at the AR and MA roots to check for approximate canceling - if the roots are numerically close. Use the simpler model unless there is compelling evidence to use a more complex model.}
...
\todo[inline]{For ARMA(2,1) and ARMA(3,1), the AIC is different by more than 2 $\implies$ ARMA(3,1) is too large to be possible.}
