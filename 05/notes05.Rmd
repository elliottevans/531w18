...
=======
date: "2018-01-23"
>>>>>>> 46d082a97f35556dbfe41d7843437eba9b0e1627
output:
  pdf_document:
...
>>>>>>> 7934be39ef53a1da37e76cf552045831142648b0
...
for(j in 1:J){
   ...
   theta[j,] <- coef(arima(Yj,order=c(2,0,1),method="ML"))
}
...
t1 <- system.time(
  huron_sim <- foreach(j=1:J) %dopar% {
     Yj <- arima.sim(list(ar=ar),n=length(huron_depth),sd=sigma)+intercept
     try(coef(arima(Yj,order=c(2,0,1),method="ML")))
  }
) 
#method = 'ML' not previously in code, added by me due to errors
```
